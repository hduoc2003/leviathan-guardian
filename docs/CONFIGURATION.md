...
| `GUARDIAN_ACK_SECRET_AUTOGEN` | `false` | Only meaningful with `GUARDIAN_ACK_SECRET_PROVIDER=file`. `true` mints a missing key file (mode `0600`, parent directories created) instead of failing startup, so a deployment whose keystore sits on persistent encrypted storage generates its identity on first boot and reads it back afterwards. Off by default because it converts a wrong path or an unmounted volume from a startup error into a **new** identity, which freezes every account pinned to the previous ack-key commitment. Case-insensitive `true`/`false` (blank counts as `false`); any other value fails startup. Every boot logs an `ack signers` line carrying `falcon_commitment` and `ecdsa_commitment`. |
